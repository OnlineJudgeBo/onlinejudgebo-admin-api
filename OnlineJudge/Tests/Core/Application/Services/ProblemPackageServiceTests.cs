...
public class ProblemPackageServiceTests
{
    ...
    public async Task ExportProblemPackageAsync_CopiesSecretTestData_ExcludesSampleFiles_KeepsJudgeNativeExtensions()
    {
        ...
        fileManager.Setup(item => item.ListFiles("5"))
            .Returns(new List<string> { "1.in", "1.out", "sample.in", "sample.out", "sample2.in", "sample2.out", "checker-notes.txt" });
        ...
        Assert.Null(FindEntry(zip, "data/secret/sample2.in"));
        Assert.Null(FindEntry(zip, "data/secret/sample2.out"));
        ...
        fileManager.Verify(item => item.ReadFile("5", "sample2.in"), Times.Never);
        fileManager.Verify(item => item.ReadFile("5", "sample2.out"), Times.Never);
    }
